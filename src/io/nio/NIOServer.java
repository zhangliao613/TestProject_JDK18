...
public class NIOServer {
    ...
    public static class ReactorTask implements Runnable {
        ...
        public ReactorTask() {
            try {
            	System.out.println("===ReactorTask===cs===begin===");
                ...
                System.out.println("===ReactorTask===cs===end===");
        ...
        public void run() {
            ...
            while (true) {
                try {
//                	System.out.println("===ReactorTask===run===begin==="+new Date());
                    selector.select(1000);
                    Set<SelectionKey> selectedKeys = selector.selectedKeys();
                    Iterator<SelectionKey> it = selectedKeys.iterator();
                    SelectionKey key = null;
//                    System.out.println("===ReactorTask===run===it.hasNext()==="+it.hasNext());
                    while (it.hasNext()) {
                        key = it.next();
                        it.remove();
                        try {
                            if (key.isValid()) {
                                // 处理新接入的请求消息
                                if (key.isAcceptable()) {
                                	System.out.println("===ReactorTask===run===key.isAcceptable()==="+key.isAcceptable());
